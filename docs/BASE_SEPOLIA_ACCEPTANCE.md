...
- Saved local payment and operator-only modes: disabled
- Venice native API-key ceiling: conservatively bounded
...
- Settlement balance reconciliation passed across all release calls
...
The acceptance runner uses a new ephemeral buyer key held only in process memory, verifies chain, asset, recipient, scheme, and exact price before signing, then reconciles buyer and settlement-account balances.
